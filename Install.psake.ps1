...
Task Install -Depends RegisterPSRepository -Precondition { $Mode -eq  'Install'}  {
  ...
  $PSGallery.Modules |% {
    ...
    PowershellGet\Install-Module -Name $_ -Repository PSGallery -Scope AllUsers  -SkipPublisherCheck -verbose -AllowClobber
  }

  Write-Host "MyGet"
  $MyGet.Modules  |% {
    Write-Host "Install module $_"
    PowershellGet\Install-Module -Name $_ -Repository OttoMatt -Scope AllUsers -verbose -AllowClobber
  }
